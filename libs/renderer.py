    ...
    def run(self):
        ...
        while not panic:
            ...
            try:
                ...
                curs.execute(
                    logQ(
                        "SELECT name, st_asgeojson(position),rotation, typ, controller, hp, modification FROM object WHERE typ != 'track' "
                    )
                )
                ...
                for r in objects:
                    ...
                    if (
                        not name in self.objectman.objectlist
                        or typ != self.objectman.objectlist[name].typ
                    ):
                        print("SPAWN %s" % (name,))
            ...
            try:
                if age % 50 == 30:
                    curs.execute(
                        logQ(
                            "DELETE from object where name in (SELECT name FROM object WHERE typ = 'track' and age(now(), modification) > interval'4seconds' order by modification desc);"
                        ),
                    )
                ...
                if age % 50 == 10:
                    ...
                    curs.execute(
                        logQ(
                            """SELECT unaccent(coalesce(name, ref))
    ...
    def run(self):
        ...
        while not panic:
            ...
            try:
                logQ(
                    "SELECT tick(%s))"
                )  ##Still havent migrated the tick to a function.. Lets pretend for demonstrational purposes
                cursor.execute(
                    """UPDATE object o SET 
               inertia=
        ...
        while not panic:
            ...
            try:
                ...
                if 1:
                    ...
                    curs.execute(
                        logQ(
                            "SELECT x, y, zoom, data FROM frontendcache WHERE x = %s and y = %s and zoom=%s"
                        ),
                        (_pos[0], _pos[1], zoom),
                    )
                    d = curs.fetchone()
...
class game:
        ...
        for _x in range(0, int(dx), self.rastermap.pixpermap[0] * size):
            ...
            for _y in range(0, int(dy), self.rastermap.pixpermap[1] * size):
                for readahead in range(1, 8 - len(self.rastermap._requestqueue)):
                    self.rastermap.get(
                        (_x + x, _y + y + readahead * self.rastermap.pixpermap[1] * size)
                    )
                ...
                self._map.blit(
                    pygame.transform.scale(s, (resx, resy)),
    ...
    def generateMap(self):
        ...
        curs.execute(
            logQ(
                """select unaccent(name), round(st_area(way)), st_asgeojson( way ), st_asgeojson( st_centroid(way) ),'polygon'  from (SELECT * FROM planet_osm_polygon where boundary = 'administrative' and name not ilike '%kreis%'  order by round(st_area(way)) desc limit 50 ) a
		UNION ALL
...
"""
        ...
        for region in regions:
            ...
            pygame.draw.polygon(
                self._overviewmap, (20 + random() * 50, 20 + random() * 50, 20 + random() * 50), pts
            )
...
"""
        ...
        for region in regions:
            ...
            center = ((center[0] - smallest[0]) * scale[0], (center[1] - smallest[1]) * scale[1])
            pygame.draw.polygon(
                self._map, (20 + random() * 50, 20 + random() * 50, 20 + random() * 50), pts
            )
        ...
        try:
            ...
            for event in pygame.event.get():
                ...
                if pygame.key.get_pressed()[pygame.K_r]:
                    ...
                    curs.execute(
                        logQ(
                            " WITH del AS ( DELETE FROM OBJECT RETURNING * ) SELECT SPAWN(del.controller, 'pedestrian') FROM del WHERE controller is not null;"
                        )
                    )
                ...
                if pygame.key.get_pressed()[pygame.K_t]:
                    ...
                    # curs.execute(" SELECT SPAWN(object.controller, 'pedestrian') FROM object WHERE controller is not null;;", (self.username,) )
                    curs.execute(
                        " UPDATE OBJECT SET CONTROLLER = NAME WHERE NAME = %s;", (self.username,)
                    )
            ...
            if pygame.key.get_pressed()[pygame.K_UP]:
                pass
                curs.execute(
                    logQ("UPDATE object set acceleration = acceleration + 1 where controller = %s"),
                    (self.username,),
                )
    ...
    def ToCenter(self, pos):
        ...
        return (
            (pos[0] - rendercenter[0]) / zoom + self.size[0] / 2,
    ...
    def finishFrame(self):
        ...
        try:  # Very quickly prototyped. things may fail alot due to the lack of proper handling
            try:
                if (
                    self.objectmanager.objectlist[self.lastcontrolledobject].controller
                    == self.username
                ):
                    pass
                else:
        ...
        rendercenter = self.userpos
        renderrect_lower = (
            rendercenter[0] - (self.size[0] / 1.0) * zoom,
            rendercenter[1] - (self.size[1] / 1.0) * zoom,
        )
        renderrect_upper = (
            rendercenter[0] + (self.size[0] / 1.0) * zoom,
            rendercenter[1] + (self.size[1] / 1.0) * zoom,
        ...
        imgsize = (self.rastermap.pixpermap[0] * zoom, self.rastermap.pixpermap[1] * zoom)
        loopcount = 0
        ...
        self.screen.blit(
            self.kreis.image,
            (
                self.screen.get_width() / 2 - self.kreis.image.get_width() / 2,
                self.screen.get_height() / 2 - self.kreis.image.get_height() / 2,
            ),
        )
        ...
        try:
            ...
            for drawable in drawlist:
                ...
                blitim = pygame.transform.smoothscale(
                    blitim,
                    (int(blitim.get_width() / (4 * zoom)), int(blitim.get_height() / (4 * zoom))),
                )
                pos = self.ToCenter(obj.position)
                self.screen.blit(
                    blitim, (pos[0] - blitim.get_width() / 2.0, pos[1] - blitim.get_height() / 2.0)
                )
                if obj.typ != "track":
                    self.putText(
                        "%s [%s]" % (drawable, int(obj.hp)),
                        self.screen,
                        (pos[0] - 10, pos[1] + 20),
                        (0, 0, 255),
                    )
        ...
        try:
            self.putText(
                "FPS %s" % (int(self.clock.get_fps())), self.screen, (100, 25), (255, 10, 10)
            )

            self.putText(
                "%s" % (self.objectmanager.currentstreet,),
                self.screen,
                (self.size[0] / 2 - 100, self.size[1] / 2 + 100),
                (255, 0, 0),
            )
        except:
        ...
        try:
            ...
            for element in querylog:
                # continue
                offset += 15
                self.putText("%s" % (element), self.screen, (10, 790 - offset), (255, 255, 255))
        except Exception as e:
            print(e)
            pass
